...
d3.csv("data/brecha.abstencion.sc.generales-spain-2016.csv", function(error, zonas) {
  ...
  x.domain(dimensions = d3.keys(zonas[0]).filter(function(d) {
    ...
    return d != "zona" && d != "municipio" && d != "municipio" && d != "provincianombre" && (y[d] = d3.scale.linear() //removes from chart variables
  ...
  foreground = svg.append("g")
    ...
    .enter().append("path")
      .attr("d", path)
      .attr("class",function(d) { return replacement(d.zona) + " "+ d.provincia + " "+ d.municipio;} ) // colorea líneas según color de provincia
      .attr("stroke","#000")
      .attr("fill","none")
